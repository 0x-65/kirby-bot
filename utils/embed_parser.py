...
async def to_object(params):
    ...
    for part in get_parts(params):
        ...
        if part.startswith('react:'):
            reactions = part[len('react:'):]

        if part.startswith('attach: '):
            file=part[len('attach: '):]
            async with aiohttp.ClientSession() as session:
                async with session.get(file, headers=headers) as resp:
                    obj = io.BytesIO(await resp.read())
            xd = discord.File(obj, filename=f"kirby{'.mp4' if '.mp4' in file else '.png'}")

        if part.startswith('http: '):
            url=part[len('http: '):].split(' && ')
            async with aiohttp.ClientSession() as session:
    ...
    return content, embed, view, reactions, xd, http
...
async def get_artist_playcount(user: str, artist: str):
        async with aiohttp.ClientSession() as cs:
            async with cs.get(
                f"https://ws.audioscrobbler.com/2.0/?method=artist.getInfo&api_key=e739760b740efae08aeef62f0e15d7b7&artist={artist.replace(' ', '+')}&format=json&username={user}"
            ) as response:
                r = await response.json()
        return r["artist"]["stats"]["userplaycount"]


async def get_artist_info(user: str, artist: str):
        async with aiohttp.ClientSession() as cs:
            async with cs.get(
                f"https://ws.audioscrobbler.com/2.0/?method=artist.getInfo&api_key=e739760b740efae08aeef62f0e15d7b7&artist={artist.replace(' ', '+')}&format=json&username={user}"
            ) as response:
